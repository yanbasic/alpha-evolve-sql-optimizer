"""AlphaEvolve evaluator — MySQL engine.
...
# Reverse lookup for pymysql FIELD_TYPEs
TYPE_NAME_MAP = {v: k for k, v in FIELD_TYPE.__dict__.items() if isinstance(v, int)}
...
def _get_column_signature(conn, sql: str, timeout_ms: int) -> tuple[tuple[str, str], ...]:
    ...
    with conn.cursor() as cur:
        ...
        return tuple((c[0], TYPE_NAME_MAP.get(c[1], f"type_{c[1]}")) for c in cols)
